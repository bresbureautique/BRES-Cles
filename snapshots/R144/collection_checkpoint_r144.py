...
def export_checkpoint(state: Dict[str, Any], work_queue: Dict[str, Any], current_correction_sheet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item_id in queue_ids:
        item = by_id[item_id]
        mutable_items.append({"work_item_id": item_id,"status": item.get("status"),"block_reason": item.get("block_reason"),"status_change_count": int(item.get("status_change_count") or 0)})
    next_action = hierarchy.get("global_progress", {}).get("next_executable_action")
    checkpoint: Dict[str, Any] = {
        "schema": _CHECKPOINT_SCHEMA,"milestone": "BRES Cles catalogue audit R144","source_state_schema": reconciled.get("schema"),"source_queue_schema": work_queue.get("schema"),"source_correction_sheet_schema": current_correction_sheet.get("schema"),"source_dependency_schema": dependency.get("schema"),"source_hierarchy_schema": hierarchy.get("schema"),"source_queue_sha256": _canonical_sha256(work_queue),"source_correction_sheet_sha256": _canonical_sha256(current_correction_sheet),"reconstructed_state_sha256": _canonical_sha256(reconciled),"dependency_snapshot_sha256": _canonical_sha256(dependency),"hierarchy_snapshot_sha256": _canonical_sha256(hierarchy),"active_test_runtime": "V2.28 TEST R116","stable_version": "V2.27","critical_pair_count": 17,"work_item_count": len(mutable_items),"mutable_items": mutable_items,"history": copy.deepcopy(reconciled.get("history", [])),"next_event_sequence": int(reconciled.get("next_event_sequence") or 1),"status_counts": copy.deepcopy(reconciled.get("status_counts", {})),"completed_item_count": int(reconciled.get("completed_item_count") or 0),"redo_item_count": int(reconciled.get("redo_item_count") or 0),"blocked_item_count": int(reconciled.get("blocked_item_count") or 0),"active_issue_count": int(reconciled.get("active_issue_count") or 0),"resolved_issue_count": int(reconciled.get("resolved_issue_count") or 0),"global_progress_state": hierarchy.get("global_progress", {}).get("progress_state"),"global_next_executable_work_item_id": None if next_action is None else next_action.get("work_item_id"),"finalisation_or_verification_item_count": dependency.get("finalisation_or_verification_item_count"),"dependency_ready_count": dependency.get("dependency_ready_count"),"dependency_blocked_count": dependency.get("dependency_blocked_count"),"work_item_ids_preserved_from_r140": True,"checkpoint_is_documentary_only": True,"resume_requires_same_r140_queue": True,"resume_requires_same_r139_correction_sheet": True,"counts_as_real_physical_evidence": False,"evidence_credit": 0,"measurements_used_for_checkpoint": False,"recognition_score_used_for_checkpoint": False,"decision_authority": "NONE","policy_status": "A_CONFIRMER","canonical_catalogue_write_allowed": False,"runtime_mutation_allowed": False,"automatic_validation_allowed": False,"acceptance_threshold_authorized": False,"candidate_selection_allowed": False,"validated_reference": None}
...
def import_checkpoint(checkpoint: Dict[str, Any], work_queue: Dict[str, Any], current_correction_sheet: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if checkpoint.get("source_queue_sha256") != _canonical_sha256(work_queue):
        raise ValueError("r144_source_queue_drift")
    if checkpoint.get("source_correction_sheet_sha256") != _canonical_sha256(current_correction_sheet):
    ...
    for compact in mutable_items:
        ...
        target["status"] = status; target["block_reason"] = block_reason if status == "BLOQUEE" else None; target["status_change_count"] = status_change_count; target["completion_verified_against_current_sheet"] = status == "REALISEE" and not target["issue_active"]
    ...
    base["history"] = copy.deepcopy(history); base["next_event_sequence"] = int(checkpoint.get("next_event_sequence") or 0); base = r141._refresh_summary(base); r141._state_guard(base, work_queue)
    reconciled = r142.reconcile_dependency_state(base, work_queue, current_correction_sheet)
    if _canonical_sha256(reconciled) != checkpoint.get("reconstructed_state_sha256"):
    ...
    return {"schema": "bres-live-critical-pair-collection-checkpoint-import-r144-v1","state": reconciled,"dependency_report": dependency,"progress_hierarchy": hierarchy,"checkpoint_payload_sha256": stored_payload_hash,"exact_state_reconstructed": True,"counts_as_real_physical_evidence": False,"evidence_credit": 0,"decision_authority": "NONE","canonical_catalogue_write_allowed": False,"runtime_mutation_allowed": False,"automatic_validation_allowed": False,"candidate_selection_allowed": False,"validated_reference": None}
