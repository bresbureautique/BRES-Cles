...
_CHAIN_SCHEMA = "bres-live-critical-pair-collection-checkpoint-chain-r146-v1"
...
def audit_checkpoint_chain(checkpoints: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not isinstance(checkpoints, list) or not checkpoints:
        raise ValueError("r146_checkpoint_chain_empty")
    ...
    if work_item_count != 1462 or len(item_order) != 1462:
        raise ValueError("r146_work_item_count_invalid")

    checkpoint_rows: List[Dict[str, Any]] = []
    for index, checkpoint in enumerate(checkpoints):
        if checkpoint.get("source_queue_sha256") != queue_hash:
            raise ValueError(f"r146_source_queue_drift:{index}")
        if checkpoint.get("work_item_count") != work_item_count:
            raise ValueError(f"r146_work_item_count_drift:{index}")
        ids = [row.get("work_item_id") for row in checkpoint.get("mutable_items", [])]
        if ids != item_order:
            raise ValueError(f"r146_work_item_order_or_set_drift:{index}")
    ...
    report: Dict[str, Any] = {
        ...
        "milestone": "BRES Cles catalogue audit R146",
