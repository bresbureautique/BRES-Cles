/* BRES Clés V2.28 TEST — recherche catalogue Silca massif (V3) */
(function(){
  ...
  function render(results,box){
    ...
    if(top.m.score>=88 && margin>=8) h+=`<b>Candidat fort</b>, mais V2.28 TEST ne le valide pas seule : contrôle du marquage/profil requis.<br>`;
    else h+=`<b>Résultat à confirmer.</b> Écart entre les meilleurs candidats : ${margin} point(s).<br>`;
    ...
    box.className='status '+((top.m.score>=88&&margin>=8)?'ok':'warn');box.innerHTML=h;
  }
  function addCard(){
  ...
  try{
    document.title='BRES Clés V2.28 TEST';
    const badge=document.querySelector('.app-version-badge');if(badge)badge.textContent='V2.28 TEST • CATALOGUE SILCA MASSIF • V2.27 CONSERVÉE';
